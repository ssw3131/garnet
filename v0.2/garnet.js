...
// 보정패치 :
	W.console = W['console'] ? W['console'] : {log : function(){}}, W.log = W['log'] ? W['log'] : function(){ W.console.log( arguments[0] ) }, W.JSON = W['JSON'] ? W['JSON'] : {parse : function( $v ){ return ( 0, eval )( '(' + $v + ')' ); }}, Date.now = Date.now * 1 || function(){ return +new Date }, W.requestAnimFrame = (function(){ return W.requestAnimationFrame || W.webkitRequestAnimationFrame || W.mozRequestAnimationFrame || function( $loop ){ W.setTimeout( $loop, 17 ) } })(), (function( f ){ W.setTimeout = f( W.setTimeout ), W.setInterval = f( W.setInterval ) })( function( f ){
		return function( $a, $b ){
...
// CORE :
		dk.fn = function( $k, $v ){
			$k = $k.replace( trim, '' ), $k = $k.charAt( 0 ).toLowerCase() + $k.substring( 1, $k.length ), dk[$k] ? dk.err( 'dk.fn에 이미 ' + $k + '값이 존재합니다' ) : dk[$k] = $v;
		}, dk.cls = function( $k, $v ){
		$k = $k.replace( trim, '' ), $k = $k.charAt( 0 ).toUpperCase() + $k.substring( 1, $k.length ), dk[$k] ? dk.err( 'dk.cls에 이미 ' + $k + '값이 존재합니다' ) : dk[$k] = $v;
	}, dk.obj = function( $k, $v ){
		$k = $k.replace( trim, '' ).toUpperCase(), dk[$k] ? dk.err( 'dk.obj에 이미 ' + $k + '값이 존재합니다' ) : dk[$k] = $v;
	},

// INFO :
		dk.obj( 'INFO', {
			name : 'Dk garnet', version : 'v0.2.0', github : 'https://github.com/ssw3131/garnet.git'
		} ),
...
// DETECTOR :
		dk.obj( 'DETECTOR', (function( $w, $doc ){
			var navi = $w.navigator, agent = navi.userAgent.toLowerCase(), platform = navi.platform.toLowerCase(), app = navi.appVersion.toLowerCase(), device = 'pc', os, osv, browser, bv, flash, prefixCss, prefixStyle, transform3D, keyframe = $w['CSSRule'], docMode = 0, d = $doc.createElement( 'div' ), s = d.style, c = $doc.createElement( 'canvas' ), a = $doc.createElement( 'audio' ), v = $doc.createElement( 'video' ), t0, ie, chrome, firefox, safari, opera, naver;
			...
			if( agent.indexOf( 'android' ) > -1 ){
				...
				if( agent.indexOf( 'mobile' ) == -1 ){
					browser += 'Tablet', device = 'tablet';
				}
				else{
					device = 'mobile';
				}
				if( t0 = /android ([\d.]+)/.exec( agent ) ){
					t0 = t0[1].split( '.' ), osv = parseFloat( t0[0] + '.' + t0[1] );
				}
				else{
					osv = 0;
				}
				...
				if( t0 = /os ([\d_]+)/.exec( agent ) ){
					t0 = t0[1].split( '_' ), osv = parseFloat( t0[0] + '.' + t0[1] );
				}
				else{
					osv = 0;
				}
				...
				if( platform.indexOf( 'win' ) > -1 ){
					...
					if( agent.indexOf( t0 + '5.1' ) > -1 ){
						osv = 'xp';
					}
					else if( agent.indexOf( t0 + '6.0' ) > -1 ){
						osv = 'vista';
					}
					else if( agent.indexOf( t0 + '6.1' ) > -1 ){
						osv = '7';
					}
					else if( agent.indexOf( t0 + '6.2' ) > -1 ){
						osv = '8';
					}
					...
					os = 'mac', t0 = /os x ([\d._]+)/.exec( agent )[1].replace( '_', '.' ).split( '.' ), osv = parseFloat( t0[0] + '.' + t0[1] ), safari() || chrome() || firefox() || opera();
				}
				else{
			...
			(function(){
				...
				if( browser == 'ie' ){
					try{ t0 = new ActiveXObject( 'ShockwaveFlash.ShockwaveFlash' ).GetVariable( '$version' ).substr( 4 ).split( ',' ), flash = parseFloat( t0[0] + '.' + t0[1] ); }catch( e ){ }
				}
				else if( ( t0 = plug['Shockwave Flash 2.0'] ) || ( t0 = plug['Shockwave Flash'] ) ){
					t0 = t0.description.split( ' ' )[2].split( '.' ), flash = parseFloat( t0[0] + '.' + t0[1] );
				}
			...
			if( keyframe ){
				if( keyframe.WEBKIT_KEYFRAME_RULE ){
					keyframe = '-webkit-keyframes';
				}
				else if( keyframe.MOZ_KEYFRAME_RULE ){
					keyframe = '-moz-keyframes';
				}
				else if( keyframe.KEYFRAME_RULE ){
					keyframe = 'keyframes';
				}
				else{
					keyframe = null;
			...
			return {
				device : device, browser : browser, browserVer : bv, os : os, osVer : osv, ie8 : browser == 'ie' && bv < 9, mobile : device == 'pc' ? 0 : 1, flash : flash, prefixCss : prefixCss, prefixStyle : prefixStyle, transform3D : transform3D, transform : ( prefixStyle + 'Transform' in s || 'transform' in s ) ? 1 : 0, transition : ( prefixStyle + 'Transition' in s || 'transition' in s ) ? 1 : 0, keyframe : keyframe ? 1 : 0, float : 'cssFloat' in s ? 'cssFloat' : 'styleFloat', canvas : c ? 1 : 0, canvasText : c && c['getContext'] && c.getContext( '2d' ).fillText ? 1 : 0, audio : a ? 1 : 0, video : v ? 1 : 0, videoPoster : v && 'poster' in v ? 1 : 0, videoWebm : v && v['canPlayType'] && v.canPlayType( 'video/webm; codecs="vp8,mp4a.40.2"' ).indexOf( 'no' ) == -1 ? 1 : 0, videoH264 : v && v['canPlayType'] && v.canPlayType( 'video/mp4; codecs="avc1.4D401E, mp4a.40.2"' ).indexOf( 'no' ) == -1 ? 1 : 0, videoTeora : v && v['canPlayType'] && v.canPlayType( 'video/ogg; codecs="theora,vorbis"' ).indexOf( 'no' ) == -1 ? 1 : 0, insertBefore : 'insertBefore' in d ? 1 : 0, innerText : 'innerText' in d ? 1 : 0, textContent : 'textContent' in d ? 1 : 0, touchBool : 'ontouchstart' in $w ? 1 : 0, currentTarget : browser == 'firefox' ? 'target' : 'srcElement', wheelEvent : browser == 'firefox' ? 'DOMMouseScroll' : 'mousewheel', isLocalhost : location.host.indexOf( 'localhost' ) < 0 ? 0 : 1
			}
		})( W, DOC ) ),
...
// EVENT :
		dkEvent = (function( $detector ){
			...
			return function( $e ){
				return {
					nativeEvent : $e, nativeTarget : $e[t0]
				}
			}
		})( dk.DETECTOR ),
			...
			function bsSelector( doc, trim, domData ){
				...
				var compare = {
					...
					':' : (function( trim, domData ){
						var mTag = {
							'first-of-type' : 1, 'last-of-type' : 1, 'only-of-type' : 1
						}, nChild = {
							'first-child' : 'firstElementChild', 'last-child' : 'lastElementChild'
						}, enabled = {
							INPUT : 1, BUTTON : 1, SELECT : 1, OPTION : 1, TEXTAREA : 1
						}, checked = {
							INPUT : 1, radio : 1, checkbox : 1, OPTION : 2
						}, skip = {
							'target' : 1, 'active' : 1, 'visited' : 1, 'first-line' : 1, 'first-letter' : 1, 'hover' : 1, 'focus' : 1, 'after' : 1, 'before' : 1, 'selection' : 1, 'eq' : 1, 'gt' : 1, 'lt' : 1, 'valid' : 1, 'invalid' : 1, 'optional' : 1, 'in-range' : 1, 'out-of-range' : 1, 'read-only' : 1, 'read-write' : 1, 'required' : 1
						};
						return function filters( el, token ){
							...
							switch( k ){
								...
								case'last-of-type':
									...
									(t1 = mTag[k]) ? dir ? ( tname = 'DQseqFCT' + tag, ename = 'DQFCTEl' + tag ) : ( tname = 'DQseqLCT' + tag, ename = 'DQLCTEl' + tag ) : dir ? ( tname = 'DQseqFC', ename = 'DQFCEl' ) : ( tname = 'DQseqLC', ename = 'DQLCEl' );
											...
											if( !dd.DQseq || dd.DQseq != bsRseq ){
												for( i = 0; i < j; i++ ){
													...
													if( isElCld ? 1 : t0.nodeType == 1 ){
														( t2 = domData( t0 ) ).DQseq = bsRseq, t2.DQindex = t1++;
													}
												}
											}
											return t0 = dd.DQindex, v == 'even' || v == '2n' ? t0 % 2 == 0 : v == 'odd' || v == '2n+1' ? t0 % 2 == 1 : t0 == v;
										case'nth-last-child':
											if( !dd.DQseqL || dd.DQseqL != bsRseq ){
												while( i-- ){
													t0 = childs[i];
													if( isElCld ? 1 : t0.nodeType == 1 ){
														( t2 = domData( t0 ) ).DQseqL = bsRseq, t2.DQindexL = t1++;
													}
												}
											}
											return t0 = dd.DQindexL, v == 'even' || v == '2n' ? t0 % 2 == 0 : v == 'odd' || v == '2n+1' ? t0 % 2 == 1 : t0 == v;
										case'nth-of-type':
											...
											if( !dd[tname] || dd[tname] != bsRseq ){
												for( i = 0; i < j; i++ ){
													...
													if( ( isElCld ? 1 : t0.nodeType == 1 ) && t0.tagName == tag ){
														( t2 = domData( t0 ) )[tname] = bsRseq, t2[lname] = t1++;
													}
												}
											}
											return t0 = dd[lname], v == 'even' || v == '2n' ? t0 % 2 == 0 : v == 'odd' || v == '2n+1' ? t0 % 2 == 1 : t0 == v;
				...
				}, rTag = /^[a-z]+[0-9]*$/i, rAlpha = /[a-z]/i, rClsTagId = /^[.#]?[a-z0-9]+$/i, DOC = document, tagName = {}, clsName = {}, className = (function( tagName, clsName ){
					var reg = {}, r = [];
				...
				})( tagName, clsName ), bsRseq = 0, navi, chrome = ( navi = window['navigator'].userAgent.toLowerCase() ).indexOf( 'chrome' ) > -1 || navi.indexOf( 'crios' ) ? 1 : 0, mQSA = {
					' ' : 1, '+' : 1, '~' : 1, ':' : 1, '[' : 1
				}, mParent = {
					' ' : 1, '>' : 1
				}, mBracket = {
					...
					'>' : 1, '+' : 1, '~' : 1
				}, R = [], arrs = {_l : 0}, aPsibl = ['previousSibling', 'previousElementSibling'], tEl = DOC.createElement( 'ul' ), isElCld, isQSA;
				...
				tEl.innerHTML = '<li>1</li>', isElCld = tEl['firstElementChild'] && tEl['lastElementChild'] && tEl['children'] ? 1 : 0, isQSA = isElCld && DOC['querySelectorAll'] ? 1 : 0;
				return function selector( query, doc, r ){
					var sels, sel, hasParent, hasQSAErr, hasQS, t0, t1, t2, t3, i, j, k, l, m, n, el, els, hit, token, tokens;
					...
					if( i == -1 ){
						sels = arrs._l ? arrs[--arrs._l] : [], sels[0] = query, i = 1;
					}
					else{
						sels = query.split( ',' ), i = sels.length;
					}
					while( i-- ){
					...
					if( sels.length == 1 ){
						t0 = sels[0][0];
						if( ( k = t0.charAt( 0 ) ) == '#' ){
							els = arrs._l ? arrs[--arrs._l] : [], els[0] = doc.getElementById( t0.substr( 1 ) ), sels[0].shift();
						}
						else if( k == '.' ){
							...
							if( !hasParent ){
								...
								if( k == '#' ){
									sels[0].pop(), els = arrs._l ? arrs[--arrs._l] : [], els[0] = doc.getElementById( t0.substr( 1 ) );
								}
								else if( k == '.' ){
									sels[0].pop(), els = className( t0.substr( 1 ) );
								}
						...
						while( l-- ){
							...
							for( tokens = sels[l], m = 0, n = tokens.length; m < n; m++ ){
								...
								if( ( k = token.charAt( 0 ) ) == ' ' ){
									...
									hit = ( ( t0 = compare[token.charAt( 0 )] ) ? t0( el, token ) : ( token == el.tagName || token == '*' ) );
								}
								if( !hit ) break;
							}
...
// PROTOTYPE :
		dk.obj( 'PROTO', {
			connect : function( $fn/* , $obj, $obj */ ){
				...
				return {
					bgColor : t0 ? function( $v ){
						var s = this.style, t0;
						if( $v ){
							$v.indexOf( 'rgba' ) >= 0 ? ( t0 = $v.replace( 'rgba', 'rgb' ).split( ',' ), t0.pop(), $v = t0.join( ',' ) + ')' ) : null, s['backgroundColor'] = $v;
						}
						else{
						...
						var s = this.style;
						if( $v ){
							s['font-smoothing'] = $v, s[prefixCss + 'font-smoothing'] = $v;
						}
						else{
							return s['font-smoothing'];
						}
					}, opacity : t0 ? function( $v ){
						var s = this.style;
						if( $v ){
							s['opacity'] = $v, s['filter'] = 'alpha(opacity=' + ( $v * 100 ) + ')';
						}
						else{
							return s['opacity'];
						}
			...
			})( dk.DETECTOR ), tree : (function( $doc, $detector ){
				var text = $detector.innerText ? 'innerText' : 'textContent';
				return {
					'>' : function( $v ){ this.el.appendChild( $v.el ); }, '<' : function( $v ){ $v == 'body' ? $doc.body.appendChild( this.el ) : $v.el.appendChild( this.el ); }, '>-' : function( $v ){ this.el.removeChild( $v.el ); }, '<-' : function( $v ){ $v == 'body' ? $doc.body.removeChild( this.el ) : $v.el.removeChild( this.el ); }, 'html' : function( $v ){ return ( $v === undefined ) ? this.el.innerHTML : this.el.innerHTML = $v; }, '+html' : function( $v ){ return this.el.innerHTML = $v + this.el.innerHTML; }, 'html+' : function( $v ){ return this.el.innerHTML = this.el.innerHTML + $v; }, 'text' : function( $v ){ return ( $v === undefined ) ? this.el[text] : this.el[text] = $v; }, '+text' : function( $v ){ return this.el[text] = $v + this.el[text]; }, 'text+' : function( $v ){ return this.el[text] = this.el[text] + $v; }
				}
...
// DOM :
		dk.cls( 'Dom', (function( $doc, $selector, $detector ){
			var factory, Dom, uuList = {}, proto = {}, maker = $doc.createElement( 'div' ), destroyDom, listDom, parser;
			...
			destroyDom = function( $k ){
				...
				if( i == 1 ){
					...
					while( i-- ) r[i] = new Dom( $arr[i] );
				}
				return r;
			}, parser = function( $str ){
				if( $str.indexOf( '>' ) < 0 ){
					return $doc.createElement( $str.substring( 1, $str.length ) );
				}
				else{
					return ( maker.innerHTML = $str, maker ).firstChild;
				}
			},
				...
				var prefixCss = $detector.prefixCss, nopx = {
					opacity : 1, zIndex : 1, 'z-index' : 1
				};
				return function(){
					...
					while( i < j ){
						k = arguments[i++];
						if( i == j ){
							return proto[k] ? proto[k].call( this ) : k.indexOf( '@' ) > -1 ? e.getAttribute( k.replace( '@', '' ) ) : ( r = s[k], t0 = parseFloat( r ), r = isNaN( t0 ) ? r : t0 );
						}// todo parseFloat( r ) % 처리
						else{
							v = arguments[i++], proto[k] ? proto[k].call( this, v ) : k.indexOf( '@' ) > -1 ? e.setAttribute( k.replace( '@', '' ), v ) : s[k] = s[prefixCss + k] = typeof v == 'number' ? nopx[k] ? v : v + 'px' : v
						}
					}
					return this;
				}
				...
				while( i < j ){
					...
					if( i == j ){
						...
						v = arguments[i++], v === null ? delete proto[k] : proto[k] = v;
					}
				}
			};

			return factory;
		})( DOC, dk.selector, dk.DETECTOR ) ), dk.PROTO.connect( dk.Dom.fn, dk.PROTO.css, dk.PROTO.tree, dk.PROTO.event ),
...
// STYLE :
		dk.cls( 'Style', (function( $doc, $head, $detector ){
			...
			el = $doc.createElement( 'style' ), $head.appendChild( el ), sheet = el.sheet || el.styleSheet, rules = sheet.cssRules || sheet.rules,
				...
				return function(){
					...
					while( i < j ){
						k = arguments[i++];
						if( i == j ){
							return proto[k] ? proto[k].call( {style : s} ) : ( r = s[k], t0 = parseFloat( r ), r = isNaN( t0 ) ? r : t0 );
						}
						else{
							v = arguments[i++], proto[k] ? proto[k].call( {style : s}, v ) : s[k] = s[prefixCss + k] = typeof v == 'number' ? nopx[k] ? v : v + 'px' : v
						}
					}
					return this;
				}
			...
			return factory;
		})( DOC, HEAD, dk.DETECTOR ) ), dk.PROTO.connect( dk.Style.fn, dk.PROTO.css ),
...
// LOADER :
		dk.fn( 'ajax', (function( $w, $detector ){
			...
			checkXMLHttp = (function(){
				...
				params = param( arguments ), rq.open( 'GET', $url, true ), rq.setRequestHeader( 'Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8' ), rq.onreadystatechange = function(){
			...
			return ajax;
		})( W, dk.DETECTOR ) ), dk.fn( 'js', (function( $w, $doc, $head ){
		var js;
		js = (function(){
			var uuId = 0;
			return function( $cb, $url ){
				var el = $doc.createElement( 'script' ), t0, t1, id = uuId++;
				$cb ? ( t0 = $url.charAt( $url.length - 1 ) ) : 0, t1 = ( t0 == '=' ), t1 ? $w['____callbacks' + id] = function(){
		...
		return function( $cb, $url/* ,$url, $url */ ){
			...
			load = function(){ js( complete, arr[++i] ); }, complete = function(){ i == leng ? $cb ? $cb() : null : load(); }, leng == 1 ? js( $cb, arr[++i] ) : load();
		}
	})( W, DOC, HEAD ) ), dk.fn( 'img', (function( $doc ){
		return function( $cb, $src /* , $src, $src */ ){
			var arr = arguments, i = 0, leng = arr.length - 1, r = [], el, load, complete;
			load = function(){ el = DOC.createElement( 'img' ), el.src = arr[++i], r.push( el ), el.onload = complete; }, complete = function(){ i == leng ? $cb ? $cb( r ) : null : load(); }, load();
		}
	})( DOC ) ),
...
// OBJ :
				...
				S : function(){
					...
					while( i < j ){
						...
						if( i == j ){
							...
							v = arguments[i++], v === null ? delete this.list[k] : this.list[k] = v;
						}
					}
					reset.call( this );
					return v;
		...
		dk.obj( 'WIN', (function(){
			var r = {
				width : 0, height : 0, scrollTo : function( $x, $y ){ W.scrollTo( $x, $y ) }
			}
			return r;
		})() ),
		...
		dk.obj( 'RESIZE', (function( $sList, $addEvent, $delEvent, $detector, $dkWIN ){
			...
			}, start = function(){ $addEvent( W, 'resize', func ); }, end = function(){ $delEvent( W, 'resize', func ); }, r = $sList( 'RESIZE', 1, start, end ), func();
			return r;
		...
		dk.obj( 'SCROLL', (function( $sList, $addEvent, $delEvent, $doc ){
			...
			func = function(){
				r.scrollLeft = $doc.documentElement.scrollLeft ? $doc.documentElement.scrollLeft : $doc.body.scrollLeft ? $doc.body.scrollLeft : 0, r.scrollTop = $doc.documentElement.scrollTop ? $doc.documentElement.scrollTop : $doc.body.scrollTop ? $doc.body.scrollTop : 0, r['update']();
			}, start = function(){ $addEvent( W, 'scroll', func ); }, end = function(){ $delEvent( W, 'scroll', func ); }, r = $sList( 'SCROLL', 1, start, end );
			return r;
		...
		dk.obj( 'MOUSE', (function( $sList, $addEvent, $delEvent, $detector, $dkScroll, $dkEvent ){
			var r, cancelBubbling, func, start, end, oldX, oldY, startX, startY, press, move, map = {
				mousedown : 'down', mousemove : 'move', mouseup : 'up', touchstart : 'down', touchmove : 'move', touchend : 'up'
			};
			cancelBubbling = function( $e ){
				cancelBubbling = $e.stopPropagation ? function( $e ){ $e.stopPropagation(); } : $w.event ? function(){ $w.event.cancelBubble = true; } : null, cancelBubbling( $e );
				...
				if( i ){
					r.mouseX = mouseX = eTouches[0].clientX, r.speedX = mouseX - oldX, oldX = mouseX, r.pageX = mouseX + $dkScroll.scrollLeft, r.mouseY = mouseY = eTouches[0].clientY, r.speedY = mouseY - oldY, oldY = mouseY, r.pageY = mouseY + $dkScroll.scrollTop;
					while( i-- ) touchList[i] = {
						pageX : eTouches[i].pageX, pageY : eTouches[i].pageY
					};
				}
				cancelBubbling( $e ), r.touchList = touchList, evType == 'touchstart' ? ( startX = mouseX, startY = mouseY, r.moveX = r.moveY = 0, r.expandTouch = false, move = false ) : evType == 'touchmove' ? ( r.moveX = mouseX - startX, r.moveY = mouseY - startY, r.moveX != 0 && Math.abs( r.moveX ) / Math.abs( r.moveY ) > 3 && !move ? ( $e.preventDefault(), r.expandTouch = true ) : r.expandTouch ? $e.preventDefault() : null, move = true ) : evType == 'touchend' ? ( r.moveX = r.moveY = 0, r.expandTouch = false, move = false ) : null, ev.type = map[evType] ? map[evType] : evType, r['update']( ev );
			} : function( $e ){
				var mouseX, mouseY, evType = $e.type, ev = $dkEvent( $e );
				r.mouseX = mouseX = $e.clientX, r.speedX = mouseX - oldX, oldX = mouseX, r.pageX = mouseX + $dkScroll.scrollLeft, r.mouseY = mouseY = $e.clientY, r.speedY = mouseY - oldY, oldY = mouseY, r.pageY = mouseY + $dkScroll.scrollTop, evType == 'mousedown' ? ( press = 1, startX = mouseX, startY = mouseY, r.moveX = r.moveY = 0 ) : evType == 'mousemove' ? ( r.moveX = press ? mouseX - startX : 0, r.moveY = press ? mouseY - startY : 0 ) : evType == 'mouseup' ? ( press = 0, r.moveX = r.moveY = 0 ) : null, ev.type = map[evType] ? map[evType] : evType, r['update']( ev );
			}, start = function(){
				$addEvent( DOC, 'down', func ), $addEvent( DOC, 'move', func ), $addEvent( DOC, 'up', func ), $dkScroll.S( 'dkMouseScroll', function(){} );
			}, end = function(){
				$delEvent( DOC, 'down', func ), $delEvent( DOC, 'move', func ), $delEvent( DOC, 'up', func ), $dkScroll.S( 'dkMouseScroll', null );
		...
		dk.obj( 'WHEEL', (function( $sList, $addEvent, $delEvent, $detector ){
			...
			func = function( $e ){
				...
				r['update']( delta );
			}, start = function(){ $addEvent( W, $detector.wheelEvent, func ); }, end = function(){ $delEvent( W, $detector.wheelEvent, func ); }, r = $sList( 'WHEEL', 1, start, end );
			return r;
		...
		dk.obj( 'KEY', (function( $sList, $addEvent, $delEvent, $dkEvent ){
			...
			func = function( $e ){
				...
				t0 ? t0( ev ) : 0;
			}, start = function(){ $addEvent( W, 'keydown', func ); }, end = function(){ $delEvent( W, 'keydown', func ); }, r = $sList( 'KEY', 0, start, end ), list = r.list;
		...
		dk.obj( 'REG', (function(){
			return {
				numeric : function( k ){ return /^[+-]*[0-9]*\.?\d+$/.test( k ) }, stringOnly : function( k ){ return /^[^0-9]*$/.test( k ) }, stripHTMLTags : function( k ){ return k.replace( /<\/?[^\<\/]+\/?>/g, "" ) }, lineModify : function( k ){ return k.split( "\r\n" ).join( "\n" ) }, Email : function( k ){ return /^(.+)\@(.+)\.(\w+)$/.test( k ) }, ip : function( k ){ return /^[0-9][0-9]?[0-9]?\.[0-9][0-9]?[0-9]?\.[0-9][0-9]?[0-9]?\.[0-9][0-9]?[0-9]?$/.test( k ) }, url : function( k ){ return /^(https?\:\/\/)(www\.)?(.+)\.(\w)+/.test( k ) && k.match( /\./g ).length > 1 }, KoreanRegistrationNumber : function( k ){ return /^[0-9]{6}-?[0-9]{7}$/.test( k ) }, empty : function( k ){
					if( !k ) return true;
					return !k.length
				}
			}
